/* Reservation Form Logic (static) — v2 (free-text employeeId & department) */
const qs = (s, el=document) => el.querySelector(s);
const qsa = (s, el=document) => Array.from(el.querySelectorAll(s));

const STATE_KEY = "booking_state_v2";
const ADMIN_PARAM = new URL(location.href).searchParams.get("admin") === "1";

let state = {
  data: {}, // date -> { slots: [{start,end,capacity}], bookings: [] }
  version: 2
};
...
function loadState() {
  try {
    ...
    if (parsed && parsed.version === 2) state = parsed;
  } catch(e) {
...
function renderSlots() {
  ...
  rec.slots.forEach(slot => {
    ...
    times.textContent = `${slot.start} ～ ${slot.end}`;
...
function submitBooking(e) {
  ...
  if (alreadyBooked(rec, selectedSlot, employeeId)) {
    showMsg("同一の社員番号で同じ時間枠の予約が既にあります。", "error");
    return;
  }
  ...
  rec.bookings.push({
    ...
    employeeId, department, name, note,
...
function exportCsv() {
  ...
  Object.entries(state.data).forEach(([date, rec]) => {
    rec.bookings.forEach(b => {
      rows.push([b.date,b.start,b.end,b.employeeId,b.department,b.name,(b.note||"").replace(/\\n/g," "),b.ts]);
    });
  });
